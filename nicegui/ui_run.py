...
def run(root: Callable | None = None, *,
    ...
    :param language: language for Quasar elements (default: `None`, which uses `'en-US'` for Quasar but does not add `lang` attribute to `<html>` tag; set explicitly to add the attribute; use `''` to explicitly opt-out even when overriding)
