...
async def test_emitted_is_cancelled_when_client_is_deleted(user: User):
    """The task awaiting an event must be cancelled when the client is deleted, e.g. after a disconnect (#6311)."""
    event = Event()
    results = []
    ...
    assert not event.callbacks, 'the callback should be unsubscribed'
